...
class ClusterStrategyV5(IStrategy):
    ...
    def cluster_borders(self, pair, n_clusters):
        ...
        dataframe = dataframe[dataframe.date >= '2024-03-08'].reset_index()
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        borders = self.cluster_borders(metadata['pair'], n_clusters=30)
        for border in borders:
            dataframe.loc[(dataframe.close > border),f'cluster'] = border
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (
                qtpylib.crossed_above(dataframe.close, dataframe[f'cluster']) #Trigger
            ),
            'enter_long'
        ] = 1
        ...
        dataframe.loc[
            (
                qtpylib.crossed_below(dataframe.close, dataframe[f'cluster']) #Trigger
            ),
            'enter_short'
        ] = 1
    ...
    def position_size(self, risk, max_stake, min_stake):
        return max(min(max_stake * (abs(self.stoploss) / risk), max_stake), min_stake)


    def trade_border(self, pair, side, rate):

        borders = self.cluster_borders(pair, n_clusters=30)
        if side == 'short':
            borders = np.flip(borders)
            return  borders[borders > rate][-2]
    ...
    def custom_stake_amount(self, pair: str, current_time: datetime, current_rate: float,
        ...
        border = self.trade_border(pair, side, last_candle.close)
        risk = 1 - border / last_candle.close if side == 'short' else 1 - last_candle.close / border

        self.dp.send_msg(f"Risk: {risk}")

        return self.position_size(abs(risk), max_stake, min_stake)
    ...
    def custom_stoploss(self, pair: str, trade: 'Trade', current_time: datetime,
        ...
        border = self.trade_border(pair, side, current_rate)

        if (side == 'short' and (border < stop)) or (side == 'long' and (border > stop)):
            trade.set_custom_data(key='stop', value=border)

        self.dp.send_msg(f"Border from stoploss: {border}")
        self.dp.send_msg(f"Stop from stoploss: {trade.get_custom_data(key='stop')}")
    ...
    def order_filled(self, pair: str, trade: Trade, order, current_time: datetime, **kwargs) -> None:
        ...
        if trade.nr_of_successful_entries == 1:
            ...
            last_candle = dataframe.iloc[-1].squeeze()
            side = 'short' if trade.is_short else 'long'
            stop = self.trade_border(pair, side, last_candle.close)
            trade.set_custom_data(key='stop', value=stop)
            self.dp.send_msg(f"Stop from order filled: {stop}")
