...
class ClusterStrategyV6(IStrategy):
    ...
    buy_params_long = IntParameter(1, 50, default=30, space="buy", optimize=True)
    buy_params_short = IntParameter(50, 100, default=70, space="buy", optimize=True)
    sell_params_long = IntParameter(50, 100, default=70, space="sell", optimize=True)
    sell_params_short = IntParameter(1, 50, default=30, space="sell", optimize=True)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (
                (qtpylib.crossed_above(dataframe['rsi'], self.buy_params_long.value))
            ),
            'enter_long'
        ] = 1
        ...
        dataframe.loc[
            (
                (qtpylib.crossed_below(dataframe['rsi'], self.buy_params_short.value))
            ),
            'enter_short'
        ] = 1
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (
                (qtpylib.crossed_above(dataframe['rsi'], self.sell_params_long.value))
            ),
            'exit_long'
        ] = 1
        ...
        dataframe.loc[
            (
                (qtpylib.crossed_below(dataframe['rsi'], self.sell_params_short.value))
            ),
            'exit_short'
        ] = 1
